...
async function setPrimaryColor(config, projectDirectory) {
    ...
    let stylesJSON = await Manifest_1.readXMLAsync({ path: stylesPath });
    ...
    try {
        await Promise.all([
            Manifest_1.writeXMLAsync({ path: colorsPath, xml: colorsJSON }),
            Manifest_1.writeXMLAsync({ path: stylesPath, xml: stylesJSON }),
        ]);
    }
    catch (e) {
